...
SGV initSGV() { return initialize_SGV(); }
...
void getProductsStartedByLetter(SGV sgv, char letter) {
    ...
    showProductsStartbyLetter(prods, letter, total,
...
void getProductSalesAndProfit(SGV sgv, char* productID, int month) {
    ...
    showProductSalesAndProfit(productID, month, res,
                              ((float)(end - start) / CLOCKS_PER_SEC));
}
...
void getProductsNeverBought(SGV sgv, int branchID) {
    ...
    showProductsNeverBought(prods, size, branchID,
                            ((float)(end - start) / CLOCKS_PER_SEC));
...
void getClientsOfAllBranches(SGV sgv) {
    ...
    showClientsOfAllBranches(prods, size,
                             ((float)(end - start) / CLOCKS_PER_SEC));
...
void getProductsBoughtByClient(SGV sgv, char* clientID) {
    ...
    showProductsBoughtByClient(clientID, res,
                               ((float)(end - start) / CLOCKS_PER_SEC));
}
...
void getSalesAndProfit(SGV sgv, int minMonth, int maxMonth) {
    ...
    showSalesAndProfit(minMonth, maxMonth, res,
                       ((float)(end - start) / CLOCKS_PER_SEC));
}
...
void getProductsBuyers(SGV sgv, char* productID, int branch) {
    ...
    showProductsBuyers(productID, branch, clientsN, clientsP, unitsN, unitsP,
                       ((float)(end - start) / CLOCKS_PER_SEC));
...
void getClientFavoriteProducts(SGV sgv, char* client, int month) {
    ...
    showClientFavoriteProducts(prods, qt, client, month, size,
                               ((float)(end - start) / CLOCKS_PER_SEC));
...
void getTopSelledProducts(SGV sgv, int limit) {
    ...
    showTopSoldProducts(prods, data, limit,
                        ((float)(end - start) / CLOCKS_PER_SEC));
...
void getClientTopProfitProducts(SGV sgv, char* client, int limit) {
    ...
    showClientTopProfitProducts(prods, qt, client, limit,
                                ((float)(end - start) / CLOCKS_PER_SEC));
...
void showFilesInfo(SGV sgv, char* clientsFilePath, char* productsFilePath,
    ...
    filesInfo(getLinesValidatedAndRead(sgv), clientsFilePath, productsFilePath,
              salesFilePath, time);
}
