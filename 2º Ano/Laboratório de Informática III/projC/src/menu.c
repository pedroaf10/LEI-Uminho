...
static char* infoLoad[10] = {
    "                                            Clients File Loaded ➤ ",
    "                                            Nº Of Lines Read From Clients "
    "File ➤ ",
    "                                            Clients Validated ➤ ",
    "                                            Products File Loaded ➤ ",
    "                                            Nº Of Lines Read From "
    "Products File ➤ ",
    "                                            Products Validated ➤ ",
    "                                            Sales File Loaded ➤ ",
    "                                            Nº Of Lines Read From Sales "
    "File ➤ ",
    "                                            Sales Validated ➤ ",
    "                                            RunTime ➤ ",
};
...
int showMenuOption() {
    ...
    while (res != 1 && res != 2) {
        ...
        printf(COR_B_VERMELHO "║██       ██║" COR_RESET);
        printf(COR_B_VERDE PISCAR
               "            【MENU】             " RESET COR_RESET);
...
void miniMenuQuery2(float time, int nPags, int pag, int size) {
    ...
    int tam = (59 - (strlen(strTime) + strlen(strnPags) + strlen(strPag) +
                     strlen(strSize) + 36));
    ...
    if (pag + 1 < 10000)
        printf(COR_B_VERDE " Nº of Products ➤ " COR_RESET);
    else
        printf(COR_B_VERDE "Nº of Products ➤ " COR_RESET);
...
void dadosClientesMes(int res[][12]) {
    ...
    for (i = 0; i < 12; i++) {
        ...
        if (i + 1 < 10)
            printf(COR_B_VERDE "  %d  " COR_RESET, i + 1);
        else
            printf(COR_B_VERDE " %d  " COR_RESET, i + 1);
...
void clientesQuery9(char** clientesN, int unidN, char** clientesP, int unidP,
                    int maior) {
    int i, j, tam;
...
void dadosQuery11(char** prods, int data[][3][2], int size, int nPags,
                  int pag) {
    int i, j, k;
    char str1[256];
    char str2[256];
...
/* -> Query 2 <- */
void showProductsStartbyLetter(char** prods, char letter, int size,
                               float time) {
    char c = 0;
...
/* -> Query 3 <- */
void showProductSalesAndProfit(char* product, int month, double res[][4],
                               float time) {
    char c = 0;
    int op = 0;
...
/* -> Query 9 <- */
void showProductsBuyers(char* productID, int branch, char** clientsN,
                        char** clientsP, int unitsN, int unitsP, float time) {
    int i, enters;
...
/* -> Query 10 <- */
void showClientFavoriteProducts(char** prods, int qt[], char* client, int month,
                                int size, float time) {
    int i, enters;
...
/* -> Query 11 <- */
void showTopSoldProducts(char** prods, int data[][3][2], int limit,
                         float time) {
    char c = 0;
...
void showClientTopProfitProducts(char** prods, double qt[], char* client,
                                 int limit, float time) {
    char c = 0;
    int i = 0, nPags = (limit / 7) + (limit % 7 ? 1 : 0);
...
void getProductAndMonth2(SGV sgv, char product[], int* month) {
    ...
    while (!valid) {
        ...
        } else if (j < 6 || !validateProduct(product) ||
                   !listOfProducts(getArrayProductsByLetter(sgv, product[0]),
                                   product)) {
            j = 0;
            c = 0;
            product[0] = '\0';
...
void getClient2(SGV sgv, char client[]) {
    ...
    while (!valid) {
        ...
        } else if (j < 5 || !validateClient(client) ||
                   !listOfClients(getClients(sgv), client)) {
            j = 0;
            c = 0;
            client[0] = '\0';
        }
