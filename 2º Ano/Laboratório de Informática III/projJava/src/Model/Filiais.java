...
public class Filiais implements IFiliais {
        ...
        for (IFilial f : filiais) {
            ...
            for (int i = 0; i < 12; i++) {
                ...
                for (List<IVenda> list : f.getVendas_clientes().get(i).values()) {
                    ...
                    if (aux.containsKey(list.get(0).getCodigoCliente())) {
                        aux.get(list.get(0).getCodigoCliente()).setQt(aux.get(list.get(0).getCodigoCliente()).getQt() + list.size());
                    } else {
                        aux.put(list.get(0).getCodigoCliente(), new Query8Aux(list.size(), list.get(0).getCodigoCliente()));
                    }
    ...
    /*
    public List<IQuery8Aux> topXClientes(int n){
        List<IQuery8Aux> res = new ArrayList<>();

        this.vendas_clientes
                .values()
                .forEach(l -> res.add(new Query8Aux((int) (l.stream().distinct().map(IVenda::getCodigoProduto)).count(),l.get(0).getCodigoCliente())));
    ...
    public List<List<Map<String, Double>>> faturacaoPorMesPorFilial() {
        ...
        for (IFilial f : filiais) {
            ...
            for (int i = 0; i < 12; i++) {
                ...
                for (Map.Entry<String, List<IVenda>> s : f.getVendas_produtos().get(i).entrySet()) {
                    if (!produtos.containsKey(s.getKey())) {
                        produtos.put(s.getKey(), s.getValue().get(0).getQt() * s.getValue().get(0).getPreco());
                    } else {
                        produtos.put(s.getKey(), produtos.get(s.getKey()) + s.getValue().get(0).getQt() * s.getValue().get(0).getPreco());
                    }
                }
                lista.add(produtos);
